...
class HumanPlayerMoon(My_Soldier_Players):
    def move(self):
        while(True):
            ...
            userinput = input("'its on YOU!:) ")
            if userinput in moves:
                return userinput
            else:
                print(colored("'Opps!that was close! be" +
                              "careful my friend ;)'", "red"))
...
class Game:
    ...
    def play_round(self):
        ...
        if beats(move1, move2):
            ...
            print(colored("WINNER WINNER CHICKEN" +
                          "DINNER!! PLAYER 1 WINS!", "green"))
            self.ply2.score += 1
        else:
            print(colored("ITS A TIE SOLDIER " +
                          "U HAVE TO BREAK IT!", "red"))

    def play_game(self):
        print("choose between;rock,paper or scissors:")
        print(colored("THE WAR HAS STARTED MOVE IT !!!", "red"))
        print("")
        for round in range(5):
            ...
            print(" Try Harder!.")
            print("")
            print(colored("AND THE FINAL SCOREEE ISSSS !:", "green"))
            print(
                f"Ply 1: {self.ply1.score}  Ply 2: {self.ply2.score}")
            print(colored("Soldier you should always" +
                          "remember that It’s hard to beat" +
                          "a person who never gives up so CHEER UP!", "blue"))
            print("")
        if self.ply1.score > self.ply2.score:
            print(colored("PLAYER 1 YOU DID IT WOHOOO", "cyan"))
        elif self.ply1.score < self.ply2.score:
            print(colored("AND THE WINNER IS >>>>> PLAYER 2 <<<<<", "cyan"))
        else:
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        thegameStyle = input(
            "Are you ready to start the war against Mr panda" +
            "team? Show me what you got  ;)!   ").lower()
